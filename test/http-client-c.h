...
/*
    Prototype functions
*/
struct http_response* http_req(char *http_headers, int http_len, struct parsed_url *purl);
...
struct http_response* http_put(char *url, char *custom_headers, char *post_data, size_t put_size);
struct http_response* http_delete(char *url, char *custom_headers);
...
/*
    Handles redirect if needed for get requests
*/
struct http_response* handle_redirect_delete(struct http_response* hresp, char* custom_headers)
{
    if(hresp->status_code_int > 300 && hresp->status_code_int < 399)
    {
        char *token = strtok(hresp->response_headers, "\r\n");
        while(token != NULL)
        {
            if(str_contains(token, "Location:"))
            {
                /* Extract url */
                char *location = str_replace("Location: ", "", token);
                return http_delete(location, custom_headers);
            }
            token = strtok(NULL, "\r\n");
        }
    }
...
/*
    Handles redirect if needed for post requests
*/
struct http_response* handle_redirect_put(struct http_response* hresp, char* custom_headers, char *put_data, size_t put_size)
{
    if(hresp->status_code_int > 300 && hresp->status_code_int < 399)
    {
        char *token = strtok(hresp->response_headers, "\r\n");
        while(token != NULL)
        {
            if(str_contains(token, "Location:"))
            {
                /* Extract url */
                char *location = str_replace("Location: ", "", token);
                return http_put(location, custom_headers, put_data, put_size);
            }
            token = strtok(NULL, "\r\n");
        }
    }
...
struct http_response* http_req(char *http_headers, int http_len, struct parsed_url *purl)
{
    ...
    while(sent < http_len)
    {
        tmpres = send(sock, http_headers+sent, http_len-sent, 0);
        if(tmpres == -1)
        {
...
struct http_response* http_delete(char *url, char *custom_headers)
{
    ...
    if(purl->path != NULL)
    {
        if(purl->query != NULL)
        {
            sprintf(http_headers, "DELETE /%s?%s HTTP/1.1\r\nHost:%s\r\nConnection:close\r\n", purl->path, purl->query, purl->host);
        }
        else
        {
            sprintf(http_headers, "DELETE /%s HTTP/1.1\r\nHost:%s\r\nConnection:close\r\n", purl->path, purl->host);
        }
    }
    else
    {
        ...
        {
            sprintf(http_headers, "DELETE / HTTP/1.1\r\nHost:%s\r\nConnection:close\r\n", purl->host);
        }
    }
    ...
    struct http_response *hresp = http_req(http_headers, strlen(http_headers), purl);

    /* Handle redirect */
    return handle_redirect_delete(hresp, custom_headers);
}
...
/*
    Makes a HTTP PUTT request to the given url
*/
struct http_response* http_put(char *url, char *custom_headers, char *put_data, size_t put_size)
{
    /* Parse url */
    struct parsed_url *purl = parse_url(url);
    if(purl == NULL)
    ...
    {
        sprintf(http_headers, "%s%s\r\n\r\n", http_headers, custom_headers);
                hlen = strlen(http_headers);
                http_headers = (char*)realloc(http_headers, hlen+put_size + 1);
                memcpy(http_headers + hlen, put_data, put_size);
    }
    else
    {
        sprintf(http_headers, "%s\r\n", http_headers);
    ...
    /* Make request and return response */
    struct http_response *hresp = http_req(http_headers, hlen + put_size, purl);

    /* Handle redirect */
    return handle_redirect_put(hresp, custom_headers, put_data, put_size);
}
